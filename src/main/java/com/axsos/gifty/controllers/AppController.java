...
public class AppController {
	...
	public String addProductToCart(@PathVariable("id") Long id, HttpSession session, Model model, @RequestParam("price") int price) {
		if(session.getAttribute("cartQuantity") == null) {
			session.setAttribute("cartQuantity", 0);
		}
		if(session.getAttribute("user_id") == null) {
			model.addAttribute("message", "you must sign in");
			Product product = aService.findProductById(id);
			model.addAttribute("product",product);
			return "product.jsp";
		}
